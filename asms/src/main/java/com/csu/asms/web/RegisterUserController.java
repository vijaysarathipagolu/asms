...
/**
 * 
 * @author vijay this controller handles request from user registration page
 */
@Controller
...
public class RegisterUserController {
	...
	/**
	 * this method handles the form request in the user registration page
	 * 
	 * @param user
	 * @param result
	 * @param session
	...
	public ModelAndView processSubmit(@ModelAttribute("registeruser") @Validated User user, BindingResult result,
			HttpSession session, Model model, final RedirectAttributes redirectAttributes) {
		...
		log.info("mv view name---" + mv.getViewName());
		return mv;

	}

	/**
	 * This method is used to handle the user details entered by the user in
	 * edit user page
	 * 
	 * @param user
	 *            user details
	 * @param result
	 *            Binding Result to bind errors object with UI *
	 * @return an Object which has redirect url's
	 */
	@RequestMapping(value = "/edit", method = { RequestMethod.POST })
	public String updateUser(HttpServletRequest request, @ModelAttribute("edituser") User user, BindingResult result,
			Model model) {
		user.setEdit(true);
		userValidator.validate(user, result);
